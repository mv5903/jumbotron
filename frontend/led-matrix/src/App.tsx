...
function App() {
  ...
  const [connectMessage, setConnectMessage] = useState<JumbotronConnectionString | null>();
  ...
  function connectTest() {
    ...
    if (ip) {
      fetch(`http://${ip}:5000/jumbotron`)
        ...
        .catch(error => {
          setConnectMessage({ text: `Error connecting to Jumbotron: ${error.message}`, type: InfoType.ERROR });
        })
    }
  }
  ...
  if (!jumbotron) {
    return (
      <div className='flex flex-col place-items-center gap-4'>
        ...
        <input ref={ipFieldRef} onKeyDown={e => { if (e.key === 'Enter') connectTest() } } type="text" placeholder="IP Address" className='input input-bordered w-full max-w-xs' />
        {
          connectMessage &&
          <p className={connectMessage.type}>
            {connectMessage.text}
          </p>  
        }
        <button onClick={() => connectTest()}>Connect</button>
        <p className='text-gray-300 text-opacity-25'>You can also press <kbd className="kbd kbd-sm">Enter</kbd> !</p>
      </div>
    )
  }
  return (
    <div>
      <Navbar jumbotron={jumbotron} setJumbotron={setJumbotronHelper} />
    </div>
  );
}
