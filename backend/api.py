...
def jumbotron_updater():
    while not thread_stop_event.is_set():
        if (Config.MATRIX is not None):
            socketio.emit('array_update', {'data': Config.MATRIX.get2DArrayRepresentation(), 'timestamp' : time.time_ns() }, namespace='/jumbotron')
...
def handle_connect(*args):
    ...
    Config.LOGGER.client_connect(f"Client connected to Jumbotron API from IP: {client_ip}")

    # Start the array updater thread only if it hasn't been started
    if not thread_started:
        Config.LOGGER.info("Starting jumbotron emitting updater thread")
        socketio.start_background_task(jumbotron_updater)
        thread_started = True
        
@socketio.on('disconnect', namespace='/jumbotron')
def handle_disconnect(*args):
    client_ip = request.remote_addr
    Config.LOGGER.client_disconnect(f"Client disconnected from Jumbotron API from IP: {client_ip}")
